...
// Returns a v128 composite literal.
func v128lit(lanes [16]byte) ast.Expr {
	elts := make([]ast.Expr, len(lanes))
	for i, b := range lanes {
		elts[i] = &ast.BasicLit{Kind: token.INT, Value: fmt.Sprintf("%#02x", b)}
	}
	return &ast.CompositeLit{Type: newID("v128"), Elts: elts}
}
