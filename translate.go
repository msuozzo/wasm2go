...
var (
	...
	//go:embed helpers/simd_arch.go
	helpersSimdArchSrc string
)
...
func translate(r io.Reader, w io.Writer) error {
	...
	if len(t.helpers) > 0 {
		...
		if *simdonly {
			if err := t.resolveHelpers(fset, "simd_arch.go", helpersSimdArchSrc); err != nil {
				return err
			}
		}
	...
	if constraint := buildConstraint(); constraint != "" {
