		...
		t.Run(op.name, func(t *testing.T) {
			for x := 0; x < 256; x++ {
				for y0 := 0; y0 < 256; y0 += 16 {
					...
					s := op.fn(b, a)
					for i := range r {
						if want := op.ref(a[i], b[i]); r[i] != want {
							t.Fatalf("%s(%d, %d): got %d, want %d", op.name, int8(a[i]), int8(b[i]), r[i], want)
						}
						if want := op.ref(b[i], a[i]); s[i] != want {
							t.Fatalf("%s(%d, %d): got %d, want %d", op.name, int8(b[i]), int8(a[i]), s[i], want)
						}
					}
				}
			}
	...
	for _, op := range ops {
		t.Run(op.name, func(t *testing.T) {
			for x0 := 0; x0 < 256; x0 += 16 {
				...
				r := op.fn(a)
				for i := range r {
					if want := op.ref(a[i]); r[i] != want {
						t.Fatalf("%s(%d): got %d, want %d", op.name, int8(a[i]), r[i], want)
					}
				}
			}
		})
...
func TestNarrow(t *testing.T) {
	t.Run("i8x16_narrow_i16x8", func(t *testing.T) {
		for x0 := 0; x0 < 65536; x0 += 8 {
			...
			s := i8x16_narrow_i16x8_s(a, b)
			u := i8x16_narrow_i16x8_u(a, b)
			for i := 0; i < 16; i++ {
				src := a
				if i >= 8 {
					src = b
...
func TestWiden(t *testing.T) {
	var a, b v128
	for i := range a {
		a[i] = uint8(0x11*i + 3) // distinct, sign bit varies
		b[i] = uint8(0xf5 - 7*i)
	}
	...
	for i := 0; i < 8; i++ {
		...
		if got, want := int16(getLane(lo, 16, i)), int16(int8(a[i])); got != want {
			t.Errorf("extend_low_s lane %d: got %d, want %d", i, got, want)
		}
		if got, want := int16(getLane(hi, 16, i)), int16(int8(a[8+i])); got != want {
			t.Errorf("extend_high_s lane %d: got %d, want %d", i, got, want)
		}
		if got, want := getLane(ul, 16, i), uint64(a[i]); got != want {
			t.Errorf("extend_low_u lane %d: got %d, want %d", i, got, want)
		}
		if got, want := getLane(uh, 16, i), uint64(a[8+i]); got != want {
		...
		if got, want := int16(getLane(ml, 16, i)), int16(int8(a[i]))*int16(int8(b[i])); got != want {
			t.Errorf("extmul_low_s lane %d: got %d, want %d", i, got, want)
		}
		if got, want := int16(getLane(mh, 16, i)), int16(int8(a[8+i]))*int16(int8(b[8+i])); got != want {
			t.Errorf("extmul_high_s lane %d: got %d, want %d", i, got, want)
		}
		if got, want := getLane(mlu, 16, i), uint64(uint16(a[i])*uint16(b[i])); got != want {
			t.Errorf("extmul_low_u lane %d: got %d, want %d", i, got, want)
		}
		if got, want := getLane(mhu, 16, i), uint64(uint16(a[8+i])*uint16(b[8+i])); got != want {
		...
		if got, want := int16(getLane(ep, 16, i)), int16(int8(a[2*i]))+int16(int8(a[2*i+1])); got != want {
			t.Errorf("extadd_pairwise_s lane %d: got %d, want %d", i, got, want)
		}
		if got, want := getLane(epu, 16, i), uint64(uint16(a[2*i])+uint16(a[2*i+1])); got != want {
...
func TestBitwise(t *testing.T) {
	...
	for i := range a {
		if got, want := v128_not(a)[i], ^a[i]; got != want {
			t.Errorf("not lane %d: got %x, want %x", i, got, want)
		}
		if got, want := v128_and(a, b)[i], a[i]&b[i]; got != want {
			t.Errorf("and lane %d: got %x, want %x", i, got, want)
		}
		if got, want := v128_andnot(a, b)[i], a[i]&^b[i]; got != want {
			t.Errorf("andnot lane %d: got %x, want %x", i, got, want)
		}
		if got, want := v128_or(a, b)[i], a[i]|b[i]; got != want {
			t.Errorf("or lane %d: got %x, want %x", i, got, want)
		}
		if got, want := v128_xor(a, b)[i], a[i]^b[i]; got != want {
			t.Errorf("xor lane %d: got %x, want %x", i, got, want)
		}
		if got, want := v128_bitselect(a, b, c)[i], a[i]&c[i]|b[i]&^c[i]; got != want {
...
func TestLanes(t *testing.T) {
	...
	for i := 0; i < 16; i++ {
		if got, want := i8x16_extract_lane_u(a, i), int32(a[i]); got != want {
			t.Errorf("i8x16_extract_lane_u(%d): got %d, want %d", i, got, want)
		}
		if got, want := i8x16_extract_lane_s(a, i), int32(int8(a[i])); got != want {
			t.Errorf("i8x16_extract_lane_s(%d): got %d, want %d", i, got, want)
		}
		r := i8x16_replace_lane(a, i, -3)
		for j := range r {
			want := a[j]
			if j == i {
				want = 0xfd
		...
		if got, want := i8x16_splat(int32(int8(a[i])))[i], a[i]; got != want {
